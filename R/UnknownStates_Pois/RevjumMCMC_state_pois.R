...
for (i in 2:(burn+N.iter)){
  ...
  if(runif(1)<0.5 || Q.dem==1){
   ...
   ##evaluate the marginal likelihood
    ...
    if (Q.dem==2){
        ###merge proposal for case 2 states
    ...
    Qe.pp[labelm[1],]<-pi.stat[1]/sum(pi.stat)*Qe.pp[labelm[1],]+pi.stat[2]/sum(pi.stat)*Qe.pp[labelm[2],]
     ...
     prop.wm<-sum(log(prod(Qe.pp[-c(labelm[1],labelm[2]),labelm[1]])))-sum(dbeta(W.qm[W.qm>0],2,2,log=TRUE))
      ...
      bet.pp[,labelm[1]]<-log(exp(beta[[i-1]][,labelm[1]])*(pi.stat[1]/sum(pi.stat))+exp(beta[[i-1]][,labelm[2]])*(pi.stat[2]/sum(pi.stat)))
  ...
  ###update inital 
  a.and.b <- 
    mclapply(split.data, forward.backward.pois, 
  ...
  for (k in 1:nstat){
    ...
    gen.fit<-glm(y~z1+z2,data=data[ss==k,],family = poisson)
    ...
    for (mh in 2:(B+1)){
      ...
      rr<-sum(dpois(data$y[ss==k],exp(cbind(1,data$z1[ss==k],data$z2[ss==k])%*%t(u)),log=TRUE))-
        sum(dpois(data$y[ss==k],exp(cbind(1,data$z1[ss==k],data$z2[ss==k])%*%mh.bet[mh-1,]),log=TRUE))
      rej <- min(exp(rr),1)
      if(runif(1) <= rej) {
        mh.bet[mh,] <- t(u)
      } else {
