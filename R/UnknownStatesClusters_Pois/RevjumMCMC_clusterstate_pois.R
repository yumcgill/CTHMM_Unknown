...
for (i in 2:(N.iter+burn)){
  ...
  for (cl in 1:num.cluster){
    ...
    if(runif(1)<0.5 || Q.dem==1){
      ...
      if (Q.dem>1){
        ...
        #prop.q2<-sum(dgamma(q.val2,shape=Qe.st[label,-label]*1000,rate=1000,log=TRUE))
      ...
      #sum(dnorm(u,mean=0,sd=p.be.sq,log=TRUE))
      ...
      if(log(runif(1)) <= jmp){
        ...
        a.and.b.s <-
          mclapply(split.data[kset.s],forward.backward.pois,
                   Qe=Qe.pp, pi=ini.pp,
                   beta=bet.pp)
        ...
        ### update beta
        obs.sum<-tapply(data$y[data$id%in%kset.s],ss1,sum)
        obs.sum[is.na(obs.sum)]<- 0
        obs.count<-table(ss1)
        beta.st<-log(rgamma(length(obs.count),obs.sum+prior.al,rate=obs.count+prior.beta))
      ...
      if (Q.dem==2){
        ...
        #labelm<-sample((pair.mm[,sample(1:(length(order.label)-1),1)]),2)
        
        
        #labelm<-sort(sample(1:(Q.dem),2))
        #phi<-ini.st[labelm[2]]
        ini.pp<-ini.st
        ...
        #q.val2<-c(Qe.pp[labelm[2],-c(labelm[1],labelm[2])])
        ...
        #prop.q2m<-sum(dgamma(q.val2,shape=Qe.pp[labelm[1],-labelm[1]]*10+1,rate=10,log=TRUE))
        #prop.q2m<-sum(dgamma(q.val2,shape=prior.n,rate=prior.r,log=TRUE))
        ...
        bet.pp<-beta.st
        bet.pp[labelm[1]]<-log(exp(beta.st[labelm[1]])*(pi.stat[1]/sum(pi.stat))+exp(beta.st[labelm[2]])*pi.stat[2]/sum(pi.stat))
  ...
  if(runif(1)<0.3 ||  num.cluster==1 || !any(duplicated(n.stat))){
    
    label<-sample(sort(unique(label.c[i-1,]))[table(label.c[i-1,])!=1],1)
    kset.s<-which(label.c[i-1,]==label)
    #Qe.ss<-as.matrix(bdiag(Q.stat.list[[label]],Q.stat.list[[label]]))
    ...
    if (sum(abs(u))>thre) {
      ...
      p.split<-log(0.7)-log(0.3)+log(num.cluster+1)+log(vap.vec[label])-sum(dbeta(phi,5,5,log=TRUE))-sum(dnorm(u,mean=0,sd=p.be.sq1,log=TRUE))
    ...
    Q.merge<- (pi.weight*(Q.stat.list[[labelm[1]]])+ (1-pi.weight)*(Q.stat.list[[labelm[2]]]))
    Q.m<-Q.stat.list
    Q.m[[labelm[1]]]<-Q.merge
    Q.m[[labelm[2]]]<-Q.merge
    
    be.merge<-log(pi.weight*exp(beta.stat.list[[labelm[1]]])+(1-pi.weight)*exp(beta.stat.list[[labelm[2]]]))
    be.m<-beta.stat.list
    ...
    p.merge<-log(0.3)-log(0.7)-log(num.cluster)-sum(log(orgw))+sum(dbeta(phi,5,5,log=TRUE))
  ...
  post.prob<-do.call(rbind,mclapply(margin.lik, function(x) x/sum(x)))
  ...
  a.and.b <-
    mclapply(1:length(split.data),function(tt){ forward.backward.pois(
  ...
  for (jj in 1:num.cluster){
    ...
    ### update beta
    obs.sum<-tapply(data$y[data$id%in%mm11],ss,sum)
    obs.sum[is.na(obs.sum)]<- 0
    obs.count<-table(ss)
    beta.list[[jj]]<-log(rgamma(length(obs.count),obs.sum+prior.al,rate=obs.count+prior.beta))
